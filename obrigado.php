...
<header class="topbar">
    <div class="container topbar-inner">
        <a href="index.php" class="logo logo-group">
...
<main class="thanks">
    <div class="container thanks-card">
        <?php if ($ehPix && !$pagoNoCartao): ?>
            ...
            <p class="muted">Aponte a câmera para o QR Code abaixo ou copie e cole o código no aplicativo de seu banco.</p>
            ...
            <div id="status-pix" class="status-pix">Aguardando a confirmação do pagamento…</div>

        <?php elseif ($pagoNoCartao): ?>
            <div class="check-icon">✓</div>
            <h1>Pagamento confirmado!</h1>
            <p>Sua inscrição na palestra <strong>IA PARA NEGÓCIOS</strong> está garantida.</p>
            <p class="muted">A confirmação foi enviada para <strong><?= htmlspecialchars($registro['email']) ?></strong>.</p>
        ...
        <?php else: ?>
            ...
            <p>Aguardando o processamento do pagamento. Você receberá a confirmação por e-mail.</p>
        <?php endif; ?>
    ...
    if (paymentId && statusEl) {
        const check = async () => {
            try {
                ...
                if (j.ok && j.pago) {
                    statusEl.innerHTML = '✓ Pagamento confirmado. A confirmação foi enviada por e-mail.';
